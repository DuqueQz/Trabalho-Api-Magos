...
CREATE TABLE magos_conquistas (
    ...
    FOREIGN KEY (mag_id) REFERENCES magos(mag_id),
    FOREIGN KEY (conq_id) REFERENCES conquistas(conq_id)
);
...
CREATE TABLE interesses (
    ...
    FOREIGN KEY (int_id_mago) REFERENCES magos(mag_id),
    FOREIGN KEY (int_id_habilidade) REFERENCES habilidades_magicas(hab_id)
);
...
CREATE TABLE trocas (
    ...
    FOREIGN KEY (troca_id_mago_ofertante) REFERENCES magos(mag_id),
    FOREIGN KEY (troca_id_mago_interessado) REFERENCES magos(mag_id)
);
...
CREATE TABLE itens_trocados (
    ...
    item_dano INT,
    item_mana INT,
    FOREIGN KEY (troca_id) REFERENCES trocas(troca_id)
);
...
CREATE TABLE mensagens (
    ...
    FOREIGN KEY (msg_id_troca) REFERENCES trocas(troca_id)
);
...
CREATE TABLE avaliacoes (
    ...
    FOREIGN KEY (ava_id_troca) REFERENCES trocas(troca_id),
    FOREIGN KEY (ava_id_mago_avaliador) REFERENCES magos(mag_id)
);
...
CREATE TABLE amizades (
    ...
    FOREIGN KEY (amiz_id_mago_1) REFERENCES magos(mag_id),
    FOREIGN KEY (amiz_id_mago_2) REFERENCES magos(mag_id)
);
